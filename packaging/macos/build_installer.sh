#!/usr/bin/env bash
# ============================================================================
#  Build a self-contained macOS installer (.pkg) for Flesh Render.
...
#
#  Prereqs: Xcode command-line tools (pkgbuild ships with them).
...
#
#  Optional env vars:
#    VERSION=2.0.0                 # installer version string
...
ART="$BUILD_DIR/FleshRender_artefacts/Release"
OUT="$ROOT/dist"
STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT

VST3="$ART/VST3/Flesh Render.vst3"
AU="$ART/AU/Flesh Render.component"
APP="$ART/Standalone/Flesh Render.app"
...
PKG="$OUT/FleshRender-$VERSION-macOS.pkg"
ARGS=(--root "$STAGE"
      --identifier com.voidcraftaudio.fleshrender.pkg
      --version "$VERSION"
      --install-location /)
